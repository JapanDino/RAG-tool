...
| S10 | UI: вкладка “Граф знаний” | done |
| S11 | Метрики качества (Hamming, F1) | done |
| S12 | Материалы для защиты | done |

## История изменений (кратко)
- 2026‑02‑02: Fixes — фильтрация vec IS NOT NULL в `/graph` и `/nodes/search`, валидация embedding_dim.
- 2026‑02‑02: S12 — Материалы для защиты (инструкция, демо, дамп/скриншоты).
- 2026‑02‑02: S11 — Метрики качества (Hamming, F1).
- 2026‑02‑02: S10 — UI: вкладка “Граф знаний”.
...
- UI “Граф знаний”: фильтры, градиенты, загрузка из `/graph`.
- Метрики качества: скрипт `scripts/evaluate_multilabel.py`, датасет `data/bloom_dataset.jsonl`.
- Материалы защиты: `docs/DEFENSE_MATERIALS.md`, `docs/DEMO_EXAMPLES.md`.
...
- UI‑вкладка «Граф знаний» на данных из БД. (done)
- Метрики качества на экспертной разметке (>=100 примеров). (done)
- Материалы для защиты (дампы/скриншоты/видео/презентация). (done)
